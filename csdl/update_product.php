            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>"
                        ...
                        <label>Image</label>
                        <input type="file" name="image" style=" width: 200px;height: 250px;box-sizing: border-box;color: #fff;background: url('http://localhost/csdl2020-phpsql-v2/image/<?php if(isset($_POST['save'])) { echo basename($_FILES['image']['name']);} else if(is_uploaded_file($_FILES['image']['tmp'])) { echo basename($_FILES['image']['name']);}  else echo $row['image']; ?>')  no-repeat;background-size: contain;" class="form-control" >
                        <input type="file" name="image" style=" width: 200px;height: 250px;box-sizing: border-box;color: #fff;background: url('http://localhost/csdl2020-phpsql-v2/image/<?php if(!is_uploaded_file($_FILES['image']['tmp_name'])) { echo basename($_FILES['image']['tmp_name']);}  else echo basename($_FILES['image']['name']); ?>')  no-repeat;background-size: contain;" class="form-control" >
                        <input type="file" name="image" style=" width: 200px;height: 250px;box-sizing: border-box;color: #fff;background: url('http://localhost/csdl2020-phpsql-v2/image/<?php echo basename($_FILES['image']['name']); ?>')  no-repeat;background-size: contain;" class="form-control" >
