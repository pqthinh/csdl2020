...
<body>
	...
	<div class="top0">
		<div class="container">
                    <div style="float: right;margin-left: 60%;">Chào bạn <?php echo $_SESSION['ten'];echo $_SESSION['id'];  ?> đến thăm trang của mình <3 || <a href="./public/dangxuat.php">Đăng xuất</a>
                        <?php if($_SESSION['ten']=='admin')  echo "<a href='./admin/temp-quantri.php'> Về trang quản trị </a>"?>
                    </div>
		</div>
...
where `sanphamkhuyenmai`.`idproduct`='".$row['idproduct']."' and DATE_ADD(`sanphamkhuyenmai`.`ngay_batdau`,INTERVAL `sanphamkhuyenmai`.`songay_hethan` DAY)> CURRENT_TIME";
 ...
 <!-- Kết thúc phần gợi ý chọn sp -->

<!-- Không có đủ thông tin để truy vấn sp lựa chọn tốt cho khách hàng
    -------BỎ-------
<div class="bestPick" >
	<div class="container" style="background-color: #e0e0e0;border-radius: 60px;padding: 30px;margin-top: 30px;">
		<div class="r-content">
...
//                        while($row = mysqli_fetch_array($result)) {
//                    ?>
                    <div class="imag" style="background: url('./image/<?php //echo $row['image'];?>') no-repeat;background-size: contain; " title="<?php //echo $row['idproduct']?>">
				<p><?php //echo $row['nameproduct']; ?>
                                    <a href="./public/show-product.php?id=<?php //echo $row['idproduct']!=null? $row['idproduct']:'null' ;?>" style="color: #fff"><i class="fa fa-eye" title="Xem ngay ->"></i></a></p>
			</div>
                        <?php // }
                        ?>
...
<div class="feedback" onclick="javascript:showfeedback('feedback0')" style="border-radius: 50%;background: url('./image/images_feeback.png') no-repeat;background-size: cover;">
   
</div>
<div id="feedback0" style="display: none;position: fixed;background-color: rgba(44, 69, 88, 0.9);width: 100%;height: 100vh;top:0">
    <div onclick="javascript:hidefeedback('feedback0')" style="background: #fff; font-size: 30px;width: 30px;height: 30px;border-radius: 50%;text-align: center;line-height: 25px;cursor: pointer;" >x</div>
    <div class="content_feedback">
        <h3 style="color: #57b846"> Gửi phản hồi </h3>
        <div style="display: flex; margin-left: 30px; margin-top: 30px;">
            <div class="feedb" onclick="javascript:openTabOfFeedback('donggop')" >Đóng góp ý tưởng</div>
            <div class="feedb" onclick="javascript:openTabOfFeedback('baoloi')">Báo cáo lỗi</div>
            <div class="feedb" onclick="javascript:openTabOfFeedback('phanhoi')">Phản hồi về sản phẩm</div>
        </div>
        <div class="content-tab">
            <div id='donggop' class="tabcontentfb" ><br><span>[Chưa hỗ trợ trong phiên bản này]</span><br><br/></div>
            <div id='baoloi' class="tabcontentfb" >
                <form method="post" action="./public/phanhoi.php" enctype="multipart/form-data">
                    <input type='file' name='anh' id='input_file' onchange="readURL(this);" />
                    <img id="blah" src="http://placehold.it/180" alt="your image" /><br>
                    <p>Mô tả lỗi bạn gặp phải</p><br>
            ...
            <div id='phanhoi' class="tabcontentfb" ><br/><span>[Chưa hỗ trợ trong phiên bản này]</span><br/><br/></div>
        </div>
        <br><br>
    </div>
</div>
<script>
    ...
    function hidefeedback(content) {
        document.getElementById(content).style.display='none';
    }
    function openTabOfFeedback(cong_viec) {
        //alert("Đang gọi hàm open !!!");
        var i, tabcontent;

        tabcontent = document.getElementsByClassName("tabcontentfb");
    ...
    window.addEventListener('paste', e => {
        document.getElementById('input_file').files = e.clipboardData.files;
    });
</script>
<footer id="gioithieu">
		<div class="container">
